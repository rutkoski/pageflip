...
package com.foxaweb.pageflip {
	...
	import com.devcafe.pageflip.Gradients;
	import flash.geom.*;
	...
	public class PageFlip {
		...
		*/
		...
		public static function computeFlip(ptd:Point,pt:Point,pw:int,ph:int,ish:Boolean,sens:int):Object{
			...
			if (hyp!=vhyp){
				var dy2:Number= pt1.y-cPoints[1].y;
				var tot2:Number= pw-cPoints[1].x;
				var drx2:Number= getDx(dy2, tot2);

				// push the before the last point
				cPoints[2]=new Point(cPoints[1].x+drx2, pt1.y);
			...
			// --------------------------------------------------------------
			//--- Shadow by ma.tthi.eu (http://ma.tthi.eu/blog/2009/05/07/pageflip-with-shadows/)
			//--- shadowMatFlippedIn = transform matrix to draw a shadow inside the fliped part and along the fold line
			//--- shadowMatFlippedOut = transform matrix to draw a shadow outside the fliped part and along the fold line
			...
			// if we have a 4 points shape
			if (hyp != vhyp) {
				sPoints[0]= cPoints[2].clone();
				sPoints[1]= new Point( (spt.x == 1) ? pw : 0, (spt.y == 1) ? 0 : ph);
				sPoints[2]= new Point( (spt.x == 1) ? pw : 0, (spt.y == 1) ? ph : 0);
				sPoints[3]= cPoints[3].clone();
			}
			else {
				sPoints[0]= cPoints[1].clone();
				sPoints[1]= new Point( (spt.x == 1) ? pw : 0, (spt.y == 1) ? ph : 0);
				sPoints[2]= cPoints[2].clone();
			}
			
			if (!ish){
				oriPoints(sPoints,spt,pw,ph);
			}
				
			// a1 = angle between the hypotenuse of the fliped part and the fold line
			a1= Math.atan2(dy, hyp*Math.sin(theta));
			a2= Math.PI/2 - a1;
			
			a3= a1;
			if (pt.y==0)a3=-a3;
			if (pt.x==0)a3=Math.PI+Math.PI-a3;
			if (!ish)a3=Math.PI-a3;
			
			
			
			shadowMatFlippedIn.a=Math.cos(a3);
			shadowMatFlippedIn.b=Math.sin(a3);
			shadowMatFlippedIn.c=-Math.sin(a3);
			shadowMatFlippedIn.d=Math.cos(a3);
			
			shadowMatFlippedOut= shadowMatFlippedIn.clone();
			
			oPointOut= (hyp != vhyp) ? cPoints[3].clone() : cPoints[2].clone();
			ordMatrixShadow(shadowMatFlippedOut, spt, opw, oph, ish, oPointOut, a3, a2);

			
			oPointIn= new Point();
			if (spt.x == 0 && spt.y == 0) { oPointIn.x= oPointOut.x+pw*Math.cos(a3); oPointIn.y= oPointOut.y+pw*Math.sin(a3); }
			if (spt.x == 1 && spt.y == 0) { oPointIn.x= oPointOut.x-pw*Math.cos(a3); oPointIn.y= oPointOut.y-pw*Math.sin(a3); }
			if (spt.x == 1 && spt.y == 1) { oPointIn.x= oPointOut.x-pw*Math.cos(a3); oPointIn.y= oPointOut.y-pw*Math.sin(a3); }
			if (spt.x == 0 && spt.y == 1) { oPointIn.x= oPointOut.x+pw*Math.cos(a3); oPointIn.y= oPointOut.y+pw*Math.sin(a3); }
			
			ordMatrixShadow(shadowMatFlippedIn, spt, opw, oph, ish, oPointIn, a3, a2);
			...
			return {cPoints:cPoints, pPoints:pPoints, sPoints:sPoints, pt:spt, matrix:mat, shadowMatFlippedIn:shadowMatFlippedIn, shadowMatFlippedOut:shadowMatFlippedOut, width:opw, height:oph};
		}
		...
		*
		*	@drawShadowFixed:Boolean			true = draw the shadow on the fixed page side
		*
		*	@drawShadowFlippedIn:Boolean		true = draw the shadow on the inside of the flipped page
		*
		*	@drawShadowFlippedOut:Boolean		true = draw the shadow on the outside of the flipped page
		*
		*
		*/
		public static function drawBitmapSheet(ocf:Object, mc:Shape, bmp0:BitmapData, bmp1:BitmapData, drawShadowFixed:Boolean, drawShadowFlippedIn:Boolean,
			drawShadowFlippedOut:Boolean, drawShadowFixed2:Boolean):void{
			...
			// draw the fixed part
			mc.graphics.beginBitmapFill(bmp0, new Matrix(), false, false);
			nb = ppts.length;
			...
			if (drawShadowFixed) {
				Gradients.drawShadowFixed(ocf, mc);
			}
			
			if (drawShadowFixed2) {
				Gradients.drawShadowFixed2(ocf, mc);
			}
			...
			if (drawShadowFlippedIn) {
				Gradients.drawShadowFlippedIn(ocf, mc);
			}

			// draw the outside shadow on the flipped side
			if (drawShadowFlippedOut) {
				Gradients.drawShadowFlippedOut(ocf, mc);
			}
		...
		*/
		...
		/**
		*	shadow matric correction
		*
		*/
		private static function ordMatrixShadow(mat:Matrix,spt:Point,opw:Number,oph:Number,ish:Boolean,oPoint:Point,gama:Number,beta:Number):void{

			if (spt.x==1 && spt.y==0){
				mat.tx=oPoint.x;
				mat.ty=oPoint.y;
				if (!ish){
					mat.tx=oPoint.x-Math.cos(gama)*opw-Math.cos(-beta)*oph;
					mat.ty=oPoint.y-Math.sin(gama)*opw-Math.sin(-beta)*oph;
				}
			}

			if (spt.x==1 && spt.y==1){
				mat.tx=oPoint.x+Math.cos(-beta)*oph;
				mat.ty=oPoint.y+Math.sin(-beta)*oph;
				if (!ish){
					mat.tx=oPoint.x+Math.cos(-beta)*oph;
					mat.ty=oPoint.y-Math.sin(-beta)*oph;
				}
			...
			if (spt.x==0 && spt.y==0){
				mat.tx=oPoint.x-Math.cos(gama)*opw;
				mat.ty=oPoint.y-Math.sin(gama)*opw;
			}
